...
* @version	$Id: class.ilSurveyEvaluationGUI.php 51392 2014-07-14 08:27:40Z jluetzen $
  ...
  function ilSurveyEvaluationGUI($a_object)
  {
	...
	*/
	function exportUserSpecificResults($export_format, $export_label, $finished_ids)
	{
		...
		/* #8211
		// PHZH: uncommented as we need that information */
		if ($this->object->getAnonymize() == ilObjSurvey::ANONYMIZE_OFF)
		{
			array_push($csvrow, $this->lng->txt("email")); // Patch: PHZH
		...
		foreach ($participants as $user_id)
		{
			...
			if ($this->object->getAnonymize() == ilObjSurvey::ANONYMIZE_OFF)
			{
				array_push($csvrow, $resultset["email"]); // Patch: PHZH
