...
class ilResultsByQuestionTableGUI extends ilTable2GUI
{
	...
	{
		// PHZH: always displaying export column as excel is always available
		if ($a_set['number_of_answers'] > 0)
		{
			//$this->tpl->setVariable("PDF_EXPORT", $a_set['output']);
			$exportLinks = $a_set['output'];
			
			// excel link
			if ($a_set['output_excel'] != null)
				$exportLinks .= ", " . $a_set['output_excel'];
				
			$this->tpl->setVariable("PDF_EXPORT", $exportLinks);
		}
