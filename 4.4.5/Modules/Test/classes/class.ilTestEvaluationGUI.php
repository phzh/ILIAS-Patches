 ...
 * 
 * @version		$Id: class.ilTestEvaluationGUI.php 54548 2014-10-24 10:27:15Z bheyser $
 * 
 * @ingroup ModulesTest
...
class ilTestEvaluationGUI extends ilTestServiceGUI
{
	...
	*/
	...
	{
		// PHZH: using 'exportFormat' parameter
		switch ($_GET["exportFormat"])
		{
			case "excel":
				include_once("./Modules/Test/classes/class.ilQuestionExport.php");				
				$export = new ilQuestionExport($_GET["qid"], $this->object);
				$export->exportUserResultsAsExcel();
				break;
	...
	public function singleResults()
	{
		...
		else
		{
			...
			foreach ($data->getQuestionTitles() as $question_id => $question_title)
			{
				...
				// PHZH: check if export to excel possible for question
				include_once("./Modules/Test/classes/class.ilQuestionExport.php");
				$canExportQuestion = ilQuestionExport::isQuestionTypeSupported($question_object);
				array_push($rows, 
					array(
						'qid'               => $question_id,
						...
						// PHZH: added link for Excel export
						'output_excel'		=> $canExportQuestion ? "<a href=\"" . $this->ctrl->getLinkTarget($this, "exportQuestionForAllParticipants"). "&exportFormat=excel\">" . $this->lng->txt("excel_export") . "</a>" : null
					)
				);
			}
			if (count($rows))
