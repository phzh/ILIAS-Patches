 ...
 * 
 * @version		$Id: class.assErrorText.php 49561 2014-04-22 11:02:24Z bheyser $
 *
...
class assErrorText extends assQuestion implements ilObjQuestionScoringAdjustable, ilObjAnswerScoringAdjustable
{
	...
	*/
	...
	// PHZH: getCorrectAnswersForExport, getResultHeadersForExport and getResultsForExport added
	/**
	 * Gets the correct answers for the question prepared for export.
	 * 
	 * @return multitype:string
	 */
	public function getCorrectAnswersForExport()
	{
		$result = array();		
		
		$result[] = array($this->lng->txt("assErrorText") . ":", $this->getErrorText());
		$result[] = "";
		
		$result["title"] = array($this->lng->txt("text_wrong"), $this->lng->txt("text_correct"), $this->lng->txt("points"));
		foreach ($this->getErrorData() as $error)
	...
	/**
	 * Gets the column headers for the question results prepared for export.
	 * 
	 * @param assQuestion $question
	 * @return multitype:string
	 */
	public function getResultHeadersForExport()
	{
		$headers = array();
	...
	/**
	 * Gets the result values for the specified user and test pass prepared for export.
	 *
	 * @param int $active_id
	 * @param int $pass
	 * @return multitype:string
	 */	
	public function getResultsForExport($active_id, $pass)
	{
		$values = $this->getSolutionValues($active_id, $pass);
		$result = array();
		$selections = array();
		if (is_array($values))
		{
