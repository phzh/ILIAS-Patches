...
<xsl:template name="MOBTag">
	...
	<xsl:choose>
        ...
        <!-- PHZH: MULTIMEDIAPORTAL-OBJECTS -->
        <xsl:when test="substring-after($data, 'mmp://') != ''">
            <iframe frameborder="0">
				<xsl:attribute name="scrolling">no</xsl:attribute>
				<xsl:attribute name="width"><xsl:value-of select="$width"/></xsl:attribute>
				<xsl:attribute name="height"><xsl:value-of select="$height"/></xsl:attribute>
                <xsl:attribute name="src">https://media.phzh.ch/medium/embedded/<xsl:call-template name="get-media-id"><xsl:with-param name="text" select="$data" /></xsl:call-template>?width=<xsl:value-of select="$width"/>&amp;height=<xsl:value-of select="$height"/>
                </xsl:attribute>
		...
		<xsl:when test = "$type='audio/mpeg' and (substring-before($data,'.mp3') != '' or substring-before($data,'.MP3') != '')">
			<audio class="ilPageAudio" height="30">
				<xsl:attribute name="src"><xsl:value-of select="$data"/></xsl:attribute>
				<!-- PHZH: changed min width of player to 360px -->
				<xsl:attribute name="width">
					<xsl:choose>
						<xsl:when test="$width &lt; 360">
		...
		<xsl:when test = "substring-before($data,'.flv') != '' or $type = 'video/mp4' or $type = 'video/webm'">
			...
			<!-- see #bug12622 -->
			<!-- PHZH: changed 'preload' attribute to 'metadata' that preview images are visible -->			
...
<!-- PHZH: function -->
<xsl:template name="get-media-id">
    <xsl:param name="text"/>
    
    <xsl:choose>
        <xsl:when test="contains($text, '/')">
            <xsl:call-template name="get-media-id">
