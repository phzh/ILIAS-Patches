...
class ilInitialisation
{
	...
	*/
	...
	public static function authenticate()
	{
		...
		// PHZH: rewrite user names as $ilAuth->start() does the login check 
		require_once("./Customizing/phzh/phzh_usernames.php");
		...
		// PHZH: added check against AD and Evento 
		// but only if authenticating against local data source 
		if ($_POST["username"] != "" && AUTH_CURRENT == AUTH_LOCAL && $current_script == "ilias.php" &&  !$ilAuth->getAuth() &&  $ilAuth->getStatus() == AUTH_WRONG_LOGIN) 
		{ 
			require_once("./Customizing/phzh/phzh_auth.php"); 
		}
		...
		// PHZH: write cookie for MMP 
		global $ilUser; 
		$cookieName = "ilias.phzh.ch"; 
		$email = $ilUser->getLogin(); 
		if ($email == "anonymous" || $ilUser->getId() == 0) 
		$email = ""; 
		$cookieValue = base64_encode(time() . "|" . $email); // seconds since (Januar 1 1970 00:00:00 GMT) 
		if ($_SERVER['SERVER_NAME'] != "localhost") 
		setrawcookie($cookieName, $cookieValue, 0, "/", ".phzh.ch", false, true); 
		else 
		setrawcookie($cookieName, $cookieValue, 0, "/", false, false, true);
